...
import { NodeValue as BaseNodeValue, OutputValue, NodeTypeIdentifier, NodeHandler, KeyMap, AbstractNode, Serializeable } from "../runner/types.js";
export type NodeValue = BaseNodeValue | NodeFactory<InputValues, OutputValuesOrUnknown>;
...
export type InputsMaybeAsValues<T extends InputValues, NI extends InputValues = InputValues> = Partial<{
    [K in keyof T]: AbstractValue<T[K]> | NodeProxy<NI, OutputValue<T[K]>> | T[K];
}> & {
    [key in string]: AbstractValue<NodeValue> | NodeProxy<NI, Partial<InputValues>> | NodeValue;
};
export type OutputsMaybeAsValues<T extends OutputValuesOrUnknown, NI extends InputValues = InputValues> = Partial<{
    [K in keyof T]: AbstractValue<T[K]> | NodeProxy<NI, OutputValue<T[K]>> | T[K] | unknown;
}> & {
    [key in string]: AbstractValue<NodeValue> | NodeProxy<NI, Partial<InputValues>> | NodeValue;
};
export type OutputValuesOrUnknown = {
...
export type NodeFactory<I extends InputValues = InputValues, O extends OutputValuesOrUnknown = OutputValuesOrUnknown> = (config?: AbstractNode<InputValues, I> | AbstractValue<NodeValue> | InputsMaybeAsValues<I>) => NodeProxy<I, O>;
export type InputsForHandler<T extends InputValues> = {
    [K in keyof T]: AbstractValue<T[K]> & PromiseLike<T[K]> & ((config?: BuilderNodeConfig) => NodeProxy);
} & {
    [key in string]: AbstractValue<NodeValue> & PromiseLike<NodeValue>;
} & PromiseLike<T>;
export type InputsForGraphDeclaration<T extends InputValues> = {
    [K in keyof T]: AbstractValue<T[K]> & ((config?: BuilderNodeConfig) => NodeProxy);
} & {
    [key in string]: AbstractValue<NodeValue>;
};
export type OutputsForGraphDeclaration<T extends OutputValuesOrUnknown, NI extends InputValues = InputValues> = ({
    [K in keyof T]: AbstractValue<T[K]> | NodeProxy<NI, OutputValue<T[K]>>;
} & {
    [key in string]: AbstractValue<NodeValue> | NodeProxy<NI, Partial<InputValues>>;
}) | PromiseLike<OutputsMaybeAsValues<T>> | void;
export type NodeProxyHandlerFunction<I extends InputValues = InputValues, O extends OutputValuesOrUnknown = OutputValuesOrUnknown> = (inputs: InputsForHandler<I>, node: AbstractNode<I, ProjectBackToOutputValues<O>>) => O | PromiseLike<O> | OutputsMaybeAsValues<O> | PromiseLike<OutputsMaybeAsValues<O>>;
export type GraphDeclarationFunction<I extends InputValues = InputValues, O extends OutputValuesOrUnknown = OutputValuesOrUnknown> = (inputs: InputsForGraphDeclaration<I>, base: {
    [key: string]: NodeFactory<InputValues, OutputValues>;
}) => OutputsForGraphDeclaration<O>;
export type Lambda<I extends InputValues = InputValues, O extends OutputValues = OutputValues> = NodeFactory<I, O> & Serializeable & ClosureNodeInterface;
...
export interface BuilderNodeInterface<I extends InputValues = InputValues, O extends OutputValues = OutputValues> extends AbstractNode<I, O> {
...
export type BuilderNodeConfig<I extends InputValues = InputValues> = Partial<InputsMaybeAsValues<I>> | AbstractValue<NodeValue> | BuilderNodeInterface<InputValues, Partial<I>> | {
    $id?: string;
};
export type ClosureNodeInterface<I extends InputValues = InputValues, O extends OutputValues = OutputValues> = Pick<BuilderNodeInterface<I, O>, "unProxy"> & Pick<NodeProxyMethods<I, O>, "in"> & Pick<AbstractValue<NodeValue>, "invoke"> & {
    getBoardCapabilityAsValue(): AbstractValue<BreadboardCapability> | Promise<BreadboardCapability>;
};
export declare abstract class AbstractValue<T extends NodeValue = NodeValue> implements PromiseLike<T | undefined> {
    abstract then<TResult1 = T | undefined, TResult2 = never>(onfulfilled?: ((value: T | undefined) => TResult1 | PromiseLike<TResult1>) | null, onrejected?: ((reason: unknown) => TResult2 | PromiseLike<TResult2>) | null): PromiseLike<TResult1 | TResult2>;
    ...
    abstract to<ToO extends OutputValues = OutputValues, ToC extends InputValues = InputValues>(to: NodeProxy<OutputValue<T> & ToC, ToO> | NodeTypeIdentifier | NodeHandler<OutputValue<T> & ToC, ToO>, config?: ToC): NodeProxy<OutputValue<T> & ToC, ToO>;
    abstract in(inputs: AbstractNode<InputValues, OutputValues> | AbstractValue<NodeValue> | InputsMaybeAsValues<InputValues>): void;
    ...
    abstract isObject(): AbstractValue<{
        [key: string]: NodeValue;
    }>;
 ...
 */
export type NodeProxy<I extends InputValues = InputValues, O extends OutputValues = OutputValues> = {
