...
export type OutputValue<T> = Partial<{
    [key: string]: T;
}>;
export type NodeHandlerFunction<I extends InputValues = InputValues, O extends OutputValues = OutputValues> = (inputs: I, node: AbstractNode<I, O>) => O | PromiseLike<O>;
export type NodeHandler<I extends InputValues = InputValues, O extends OutputValues = OutputValues> = {
    invoke?: NodeHandlerFunction<I, O>;
    describe?: NodeDescriberFunction;
    graph?: ScopeInterface;
} | NodeHandlerFunction<I, O>;
export type NodeHandlers = Record<NodeTypeIdentifier, NodeHandler<InputValues, OutputValues>>;
export interface Serializeable {
    serialize(metadata?: GraphMetadata): Promise<GraphDescriptor> | GraphDescriptor;
}
export type KeyMap = {
    [key: string]: string;
};
export interface EdgeInterface<FromI extends InputValues = InputValues, FromO extends OutputValues = OutputValues, ToI extends InputValues = InputValues, ToO extends OutputValues = OutputValues> {
...
export declare abstract class AbstractNode<I extends InputValues = InputValues, O extends OutputValues = OutputValues> implements Serializeable {
    abstract id: string;
    abstract type: string;
    abstract outgoing: EdgeInterface[];
    abstract incoming: EdgeInterface[];
    abstract configuration: Partial<I>;
    abstract addIncomingEdge(from: AbstractNode, out: string, in_: string, constant?: boolean, schema?: Schema): void;
    abstract invoke(inputs: I, dynamicScope?: ScopeInterface): Promise<O>;
    abstract describe(scope?: ScopeInterface, inputs?: InputValues, inputSchema?: Schema, outputSchema?: Schema): Promise<NodeDescriberResult | undefined>;
...
export interface InvokeCallbacks {
    stop?: (scope: ScopeInterface, state: StateInterface) => boolean | Promise<boolean>;
    before?: (scope: ScopeInterface, node: AbstractNode, inputs: InputValues) => undefined | Promise<OutputValues | undefined>;
    after?: (scope: ScopeInterface, node: AbstractNode, inputs: InputValues, outputs: OutputValues, distribution: OutputDistribution) => void | Promise<void>;
...
export interface ScopeInterface {
     ...
     */
    invokeOneRound(inputs: InputValues, node?: AbstractNode, state?: StateInterface): Promise<OutputValues>;
    /**
